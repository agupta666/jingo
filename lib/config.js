  ...
  return {
    ...
    validate: function() {
      ...
      if (!config.authentication.google.enabled &&
        ...
        !config.authentication.gitlab.enabled &&
      ...
      if (config.authentication.gitlab.enabled && (!config.authentication.gitlab.clientId || !config.authentication.gitlab.clientSecret || !config.authentication.gitlab.gitlabURL)) {
        error = "Invalid or missing authentication credentials for Gitlab (clientId, clientSecret, gitlabURL).";
        return false;
      }
